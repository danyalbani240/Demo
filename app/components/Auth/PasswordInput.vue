<template>
  <div key="password" class="relative">
    ...
    <button
      type="button"
      class="absolute inset-e-3 top-1/2 grid h-9 w-9 -translate-y-1/2 place-items-center rounded-xl border border-black/10 bg-black/5 text-sm transition active:scale-[0.98] dark:border-white/10 dark:bg-white/10"
  ...
  <Transition name="soft">
    ...
    >
      <span class="font-extrabold">{{ $t("auth.password_strength") }}:</span>
      <span
      ...
      <span class="opacity-80">{{ $t("auth.min_password_hint") }}</span>
    </div>
  </Transition>
</template>
